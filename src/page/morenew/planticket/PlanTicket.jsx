...
const PlaneTicketNew = (props) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const datakey = response.data.news.planeticket;

      setDatas(response.data.news.planeticket);
        ...
        {datas.slice(0, 1).map((item, index) => (
          <Link to={item.id} key={index}>
            <div className="frame_top_news">
              <div className="frame_title_img_news_planeticket">
                <h2>Tin tức nỗi bật</h2>
                <div className="frame_img_news_top_planeticket">
                  <img
                    src={
